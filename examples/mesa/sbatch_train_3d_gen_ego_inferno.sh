...
#SBATCH -J pi05_mesa_bimanual_lora_3d_gen_ego_train_inferno
#SBATCH -o examples/mesa/slurm_logs/train_3d_gen_ego_inferno_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_3d_gen_ego_inferno_%j.err
#
# L40S/inferno variant of sbatch_train_3d_gen_ego.sh — same training config
...
echo "[train-3d-gen-ego-inferno] node=$(hostname) gpu=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "[train-3d-gen-ego-inferno] config=pi05_mesa_bimanual_lora_3d_gen_ego exp=$EXP_NAME batch=$BATCH_SIZE"
echo "[train-3d-gen-ego-inferno] wandb: $WANDB_ENTITY/$WANDB_PROJECT_NAME/$EXP_NAME"
echo "[train-3d-gen-ego-inferno] checkpoints -> $CHECKPOINT_BASE_DIR/pi05_mesa_bimanual_lora_3d_gen_ego/$EXP_NAME"
...
echo "[train-3d-gen-ego-inferno] done."
