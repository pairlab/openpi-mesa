...
#SBATCH -J pi05_mesa_bimanual_lora_2d_gen_camdrop_train
#SBATCH -o examples/mesa/slurm_logs/train_2d_gen_camdrop_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_2d_gen_camdrop_%j.err
#
# LoRA finetune of pi0.5 (2D backbone, no Adapt3R) on the same 4-cam MimicGen
# Mesa bimanual dataset as ``pi05_mesa_bimanual_lora_3d_gen_camdrop``, keeping
# the per-sample single-camera attention-mask dropout (p=0.25). TrainConfig:
# pi05_mesa_bimanual_lora_2d_gen_camdrop.
#
# The 2D config reuses the 3D LeRobot dataset; depth + calibration features are
# loaded but ignored by the plain pi0.5 model. Norm stats are symlinked from the
# 3D config's assets (identical state/action transforms). Prereq:
#   assets/pi05_mesa_bimanual_lora_2d_gen_camdrop/mesa_bimanual_gen_2task_3d/
#
...
echo "[train-2d-gen-camdrop] node=$(hostname) gpu=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "[train-2d-gen-camdrop] config=pi05_mesa_bimanual_lora_2d_gen_camdrop exp=$EXP_NAME batch=$BATCH_SIZE"
echo "[train-2d-gen-camdrop] wandb: $WANDB_ENTITY/$WANDB_PROJECT_NAME/$EXP_NAME"
echo "[train-2d-gen-camdrop] checkpoints -> $CHECKPOINT_BASE_DIR/pi05_mesa_bimanual_lora_2d_gen_camdrop/$EXP_NAME"
...
echo "[train-2d-gen-camdrop] done."
