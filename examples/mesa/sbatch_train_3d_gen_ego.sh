...
#SBATCH -J pi05_mesa_bimanual_lora_3d_gen_ego_train
#SBATCH -o examples/mesa/slurm_logs/train_3d_gen_ego_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_3d_gen_ego_%j.err
#
# LoRA finetune of pi0.5 + Adapt3R on the same MimicGen Mesa bimanual LeRobot
# dataset as ``pi05_mesa_bimanual_lora_3d_gen_camdrop``, but the data config
# requests only the egocentric stream (image + depth + intrinsics + extrinsics
# + hand_mat). The other three views are never read from disk. TrainConfig:
# pi05_mesa_bimanual_lora_3d_gen_ego. Prereq norm stats live at
#   assets/pi05_mesa_bimanual_lora_3d_gen_ego/mesa_bimanual_gen_2task_3d/
...
#   EXP_NAME   : experiment name (default: mesa_bimanual_lora_3d_gen_ego_v1)
...
echo "[train-3d-gen-ego] node=$(hostname) gpu=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "[train-3d-gen-ego] config=pi05_mesa_bimanual_lora_3d_gen_ego exp=$EXP_NAME batch=$BATCH_SIZE"
echo "[train-3d-gen-ego] wandb: $WANDB_ENTITY/$WANDB_PROJECT_NAME/$EXP_NAME"
echo "[train-3d-gen-ego] checkpoints -> $CHECKPOINT_BASE_DIR/pi05_mesa_bimanual_lora_3d_gen_ego/$EXP_NAME"
