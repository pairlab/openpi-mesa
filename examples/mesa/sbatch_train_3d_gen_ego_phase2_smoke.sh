...
#SBATCH -J pi05_mesa_bimanual_lora_3d_gen_ego_phase2_smoke
#SBATCH -o examples/mesa/slurm_logs/train_3d_gen_ego_phase2_smoke_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_3d_gen_ego_phase2_smoke_%j.err
#
# Phase 2 smoke test on embers L40S — short window (~50 steps, batch 4, no
# wandb, --overwrite) running TrainConfig pi05_mesa_bimanual_lora_3d_gen_ego_phase2.
# Validates:
...
# Success criterion is monotone-ish loss decrease and no Traceback in the .err.
#
# Embers notes (see ~/.claude memory): `--gres=gpu:l40s:1` (plain `gpu:1` would
# backfill to V100 via embers multi-partition) and cpus-per-task=4. The inferno
# variant of this script (`..._smoke_inferno.sh`) lives alongside for the case
# where embers is congested.
#
