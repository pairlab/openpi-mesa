...
#SBATCH -J pi05_mesa_bimanual_lora_2d_gen_ego_train_h100
#SBATCH -o examples/mesa/slurm_logs/train_2d_gen_ego_h100_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_2d_gen_ego_h100_%j.err
#
# H100/embers variant of sbatch_train_2d_gen_ego.sh — same training config
...
echo "[train-2d-gen-ego-h100] node=$(hostname) gpu=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "[train-2d-gen-ego-h100] config=pi05_mesa_bimanual_lora_2d_gen_ego exp=$EXP_NAME batch=$BATCH_SIZE"
echo "[train-2d-gen-ego-h100] wandb: $WANDB_ENTITY/$WANDB_PROJECT_NAME/$EXP_NAME"
echo "[train-2d-gen-ego-h100] checkpoints -> $CHECKPOINT_BASE_DIR/pi05_mesa_bimanual_lora_2d_gen_ego/$EXP_NAME"
...
echo "[train-2d-gen-ego-h100] done."
