...
#SBATCH -J pi05_mesa_bimanual_lora_3d_gen_camdrop_train_h100
#SBATCH -o examples/mesa/slurm_logs/train_3d_gen_camdrop_h100_%j.out
#SBATCH -e examples/mesa/slurm_logs/train_3d_gen_camdrop_h100_%j.err
#
# H100/embers variant of sbatch_train_3d_gen_camdrop.sh — same training config
# (pi05_mesa_bimanual_lora_3d_gen_camdrop, batch 32, --resume), routed to the
...
echo "[train-3d-gen-camdrop-h100] node=$(hostname) gpu=$(nvidia-smi --query-gpu=name --format=csv,noheader | head -1)"
echo "[train-3d-gen-camdrop-h100] config=pi05_mesa_bimanual_lora_3d_gen_camdrop exp=$EXP_NAME batch=$BATCH_SIZE"
echo "[train-3d-gen-camdrop-h100] wandb: $WANDB_ENTITY/$WANDB_PROJECT_NAME/$EXP_NAME"
echo "[train-3d-gen-camdrop-h100] checkpoints -> $CHECKPOINT_BASE_DIR/pi05_mesa_bimanual_lora_3d_gen_camdrop/$EXP_NAME"
...
echo "[train-3d-gen-camdrop-h100] done."
